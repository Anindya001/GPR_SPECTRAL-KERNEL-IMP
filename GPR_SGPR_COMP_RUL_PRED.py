...
def plot_calibration_curve(true_vals, pred_vals, pred_std, method, base_dir, cap_no):
    ...
    try:
        ...
        # Now create the miscalibration plot as before
        plt.figure(figsize=(10, 8))

        # Compute miscalibration at each quantile
        miscalibration = np.array(observed_frequencies) - np.array(expected_frequencies)

        # Create bar plot for miscalibration
        plt.bar(expected_frequencies, miscalibration, width=0.035,
        ...
        plt.axhspan(-0.05, 0.05, alpha=0.25, color='lightgreen', label='Good (±5%)')
        plt.axhspan(-0.1, -0.05, alpha=0.25, color='khaki', label='Moderate (±10%)')
        plt.axhspan(0.05, 0.1, alpha=0.25, color='khaki')
        plt.axhspan(-0.3, -0.1, alpha=0.25, color='salmon', label='Poor (>10%)')
        plt.axhspan(0.1, 0.3, alpha=0.25, color='salmon')

        # Calculate and display miscalibration pattern
        ...
        plt.text(0.5, 0.95, pattern, transform=plt.gca().transAxes, fontsize=14, fontweight='bold',
                 ha='center', va='top',
        ...
        plt.ylabel('Miscalibration (Observed - Expected)', fontweight='bold', fontsize=14)
        plt.title(f'Miscalibration Analysis for Cap {cap_no} ({method})', fontweight='bold', fontsize=16)
        plt.grid(True, alpha=0.3, linestyle='--')

        # Set y-axis limits with a good range
        plt.ylim(-0.3, 0.3)
        plt.xlim(0, 1)
        ...
        plt.legend(loc='lower right', fontsize=12, framealpha=0.9, fancybox=True)

        # Adjust layout
        plt.tight_layout()

        # Save miscalibration plot
        ...
        try:
            logger.info(f"Generated Enhanced Calibration Curve for Cap {cap_no} ({method})")
        except NameError:
            print(f"Generated Enhanced Calibration Curve for Cap {cap_no} ({method})")
        return True

    except Exception as e:
        import traceback
        try:
            logger.error(f"Error plotting Calibration Curve for Cap {cap_no} ({method}): {e}")
        except NameError:
            print(f"Error plotting Calibration Curve for Cap {cap_no} ({method}): {e}")
        traceback.print_exc()
        return False
